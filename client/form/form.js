...
class AlaanuFile extends HTMLElement {
    ...
    render() {
        ...
        if (this._max > 1) {
            ...
            container.innerHTML = `<span>Upload</span>`;
        }
    ...
    onFileSelect(event) {
        ...
        if (files.length > 0) {
            this._files = Array.from(files).slice(0, this._max);
            this.render();
        }
    }
...
class AlaanuProfileOld extends AlaanuFile {
    constructor() {
        super();
        this._max = 1;
        this._isRound = false;
...
customElements.define('aln-profile-old', AlaanuProfileOld);
